/**
 * yt-play-video.js
 * Optimized styling for MUSTAFFA-MD
...
// Exact Newsletter and Bot Info for MUSTAFFA-MD
const NEWSLETTER_JID = "120363423997837331@newsletter";
const NEWSLETTER_NAME = "MUSTAFFA MD";
const BOT = "MUSTAFFA-MD";

const buildCaption = (video) => {
  const duration = video.timestamp || video.duration || "N/A";

  return (
    `*🎬 MUSTAFFA MD VIDEO PLAYER*\n\n` +
    `╭───────────────◆\n` +
    `│ 📑 Title: ${video.title}\n` +
    `│ ⏳ Duration: ${duration}\n` +
    `╰────────────────◆\n\n` +
...
const getContextInfo = (query = "", video = {}) => ({
  ...
  externalAdReply: {
    ...
    body: "MUSTAFFA-MD Video Downloader",
    mediaType: 1,
