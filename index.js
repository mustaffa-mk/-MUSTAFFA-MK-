/**
 * 𒋲⍟ᬼ⃟M💀⃝⃪U⛓ST۞༒A༒ FF⛓⍟ᬼ⃟A𒋲         ~⍟ᬼ⃟MD💀⃝⃪V2🕷️™ WhatsApp Bot (Advanced Version)
 * Creator: 𒋲⍟ᬼ⃟M💀⃝⃪U⛓ST۞༒A༒ FF⛓⍟ᬼ⃟A𒋲         ~⍟ᬼ⃟MD💀⃝⃪V2🕷️™
 * Features: auto-reconnect, error guards, LID-aware status handler, session loader, auto-bio, newsletter follow, anticall
 */

console.clear()
console.log("📳 Starting 𒋲⍟ᬼ⃟M💀⃝⃪U⛓ST۞༒A༒ FF⛓⍟ᬼ⃟A𒋲         ~⍟ᬼ⃟MD💀⃝⃪V2🕷️™-MD...")
    ...
    conn.ev.on('call', async (node) => {
        if (config.ANTICALL === 'true') {
            ...
            if (status === 'offer') {
                ...
                await conn.sendMessage(from, { text: "⚠️ *𒋲⍟ᬼ⃟M💀⃝⃪U⛓ST۞༒A༒ FF⛓⍟ᬼ⃟A𒋲         ~⍟ᬼ⃟MD💀⃝⃪V2🕷️™-MD AUTO-REJECT* ⚠️\n\nCalls are not allowed for this bot. Please send a text message instead." })
            }
        }
    })
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) setTimeout(() => connectToWA(), 5000)
          ...
          fs.readdirSync("./mustaffa/").forEach((plugin) => {
